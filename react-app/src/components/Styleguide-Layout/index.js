...
const StyleguideLayout = (props) => {
  ...
  const sections = getChildPlaceholder(props.rendering, 'ReactApp-jss-styleguide-layout')
    .filter((section) => getFieldValue(section, 'heading'))
    .map((section) => ({
      heading: getFieldValue(section, 'heading'),
      id: `i${section.uid.replace(/[{}]/g, '')}`,
      children: getChildPlaceholder(section, 'ReactApp-jss-styleguide-section')
  ...
  return (
    <div className="row">
      <div className="col-sm-8 col-lg-10">
        <Placeholder name="ReactApp-jss-styleguide-layout" rendering={props.rendering} />
      </div>
